        ...
        virtual void run() const
        {
...

            {
                ...
                static const std::vector<std::pair<double, double>> reference
                {
                    ...
                    std::make_pair( 6.80598,  eps),            // Im{P(q2 = 1.0, (2.0,5.0), (3.0,6.0), (4.0,7.0))}

                };
                TEST_CHECK_DIAGNOSTICS(diagnostics, reference);


                TEST_CHECK_NEARLY_EQUAL(real(nc->H_plus(-1.0)), -0.178407,  eps);
                TEST_CHECK_NEARLY_EQUAL(imag(nc->H_plus(-1.0)), -0.246544,  eps);
                TEST_CHECK_NEARLY_EQUAL(real(nc->H_plus(0.0)),   0.,        eps);
                TEST_CHECK_NEARLY_EQUAL(imag(nc->H_plus(0.0)),   0.,        eps);
                TEST_CHECK_NEARLY_EQUAL(real(nc->H_plus(4.0)),   1.02966,   eps);
                TEST_CHECK_NEARLY_EQUAL(imag(nc->H_plus(4.0)),   1.46104,   eps);
                TEST_CHECK_NEARLY_EQUAL(real(nc->H_plus(12.0)), -2.98047,   eps);
                TEST_CHECK_NEARLY_EQUAL(imag(nc->H_plus(12.0)), -4.73521,   eps);

                TEST_CHECK_NEARLY_EQUAL(real(nc->H_plus_residue_jpsi()),   0.452032,  eps);
                TEST_CHECK_NEARLY_EQUAL(imag(nc->H_plus_residue_jpsi()),   0.685209,  eps);
                TEST_CHECK_NEARLY_EQUAL(real(nc->H_plus_residue_psi2s()), -0.390092,  eps);
                TEST_CHECK_NEARLY_EQUAL(imag(nc->H_plus_residue_psi2s()), -0.565086,  eps);
            }
        }
