...
namespace eos
{
    ...
    namespace nff_onehalfplus_to_onehalfplus
     {
            ...
            public NonlocalFormFactor<nff::OneHalfPlusToOneHalfPlus>
        {
            ...
            public:
                  ...
                  const complex<double> & alpha_2) const
                {
                    ...
                    const double s_0   = this->t_0();
                    const double s_p   = 4.0 * pow(m_D0, 2.0);
                    const auto zLbL    = eos::nff_utils::z(pow(m_LamB + m_Lam, 2.0), s_p, s_0);
                    const auto z_Jpsi  = eos::nff_utils::z(m_Jpsi2,                  s_p, s_0);
                    const auto z_psi2S = eos::nff_utils::z(m_psi2S2,                 s_p, s_0);

                    return eos::nff_utils::PGvDV2020(z_Jpsi, zLbL, alpha_0, alpha_1, alpha_2) / (phi(m_Jpsi2, phiParam) * (-0.0329971)); //Eq. (3.37)
                };
                 ...
                 const complex<double> & alpha_2) const
                {
                    ...
                    return eos::nff_utils::PGvDV2020(z_psi2S, zLbL, alpha_0, alpha_1, alpha_2) / (phi(m_psi2S2, phiParam) * (0.440234)); //Eq. (3.37)
                };
                ...
                virtual complex<double> H_V_long(const double & q2) const
                {
                    ...
                    const auto z       = eos::nff_utils::z(q2,                       s_p, s_0);
                    const auto zLbL    = eos::nff_utils::z(pow(m_LamB + m_Lam, 2.0), s_p, s_0);
                    const auto z_Jpsi  = eos::nff_utils::z(pow(m_Jpsi, 2.0),           s_p, s_0);
                    const auto z_psi2S = eos::nff_utils::z(pow(m_psi2S, 2.0),          s_p, s_0);

                    const complex<double> blaschke_factor = eos::nff_utils::blaschke_cc(z, z_Jpsi, z_psi2S);

                    const unsigned phiParam[5] = {1,0,4,1,0};

                    return eos::nff_utils::PGvDV2020(z, zLbL, alpha_0, alpha_1, alpha_2) / phi(q2, phiParam) / blaschke_factor;
                }
                ...
                virtual Diagnostics diagnostics() const
                {
                    ...
                    results.add({ real(eos::nff_utils::z(q2, s_p, s_0)), "real(z(q2 = 16.0))" });
                    results.add({ imag(eos::nff_utils::z(q2, s_p, s_0)), "imag(z(q2 = 16.0))" });
    ...
    NonlocalFormFactor<nff::OneHalfPlusToOneHalfPlus>::make(const QualifiedName & name, const Parameters & p, const Options & o)
    {
        ...
        {
            // parametrizations
            std::make_pair("Lambda_b->Lambda::BRvD2021",     &nff_onehalfplus_to_onehalfplus::BRvD2021<nff::LambdabToLambda>::make),
        };
    ...
    struct Implementation<NonlocalFormFactorObservable<Process_, nff::OneHalfPlusToOneHalfPlus>>
    {
        ...
        NonlocalFormFactorPtr<nff::OneHalfPlusToOneHalfPlus> nff;

        Implementation(const Parameters & p, const Options & o, ParameterUser & u) :
            opt_formfactor(o, "formfactor", qnp::Name("BRvD2021")),
            nff(NonlocalFormFactor<nff::OneHalfPlusToOneHalfPlus>::make(QualifiedName(qnp::Prefix(Process_::label), opt_formfactor.value()), p, o))
        {
            u.uses(*nff);
        }
    ...
    {
        return real(this->_imp->nff->H_V_perp(q2));
    }
    ...
    {
        return imag(this->_imp->nff->H_V_perp(q2));
    }
    ...
    {
        return real(this->_imp->nff->H_V_long(q2));
    }
    ...
    {
        return imag(this->_imp->nff->H_V_long(q2));
    }
    ...
    {
        return real(this->_imp->nff->H_A_perp(q2));
    }
    ...
    {
        return imag(this->_imp->nff->H_A_perp(q2));
    }
    ...
    {
        return real(this->_imp->nff->H_A_long(q2));
    }
    ...
    {
        return imag(this->_imp->nff->H_A_long(q2));
    }
    ...
    {
        return real(this->_imp->nff->ratio_H_V_long(q2));
    }
    ...
    {
        return imag(this->_imp->nff->ratio_H_V_long(q2));
    }
    ...
    {
        return real(this->_imp->nff->ratio_H_V_perp(q2));
    }
    ...
    {
        return imag(this->_imp->nff->ratio_H_V_perp(q2));
    }
    ...
    {
        return real(this->_imp->nff->ratio_H_A_long(q2));
    }
    ...
    {
        return imag(this->_imp->nff->ratio_H_A_long(q2));
    }
    ...
    {
        return real(this->_imp->nff->ratio_H_A_perp(q2));
    }
    ...
    {
        return imag(this->_imp->nff->ratio_H_A_perp(q2));
    }
